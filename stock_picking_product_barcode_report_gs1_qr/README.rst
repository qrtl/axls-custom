...
printer. The same payload as a QR code sits in a 20mm square and leaves
...
- **A location**, for everything stored anywhere below it. Note that
  this is the stock location, not the shelf printed on the label: where
  locations are warehouse-wide, selecting one takes in the whole
  warehouse.
